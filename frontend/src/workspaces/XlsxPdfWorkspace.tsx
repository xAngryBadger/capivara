...
export function XlsxPdfWorkspace() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/convert/xlsx-pdf'), {
        method: 'POST',
        body: formData,
        headers: apiHeaders(),
      })
      ...
      const newName = files[0].name.replace(/\.xlsx$/i, '.pdf')
  ...
  return (
    <>
      <FileUploader accept=".xlsx" files={files} onFilesChange={setFiles} label="Arraste um arquivo XLSX" />
