...
export function PdfToImagesWorkspace() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/pdf-to-images'), {
        method: 'POST',
        body: formData,
        headers: apiHeaders(),
      })
      ...
      const newName = files[0].name.replace(/\.pdf$/i, '_images.zip')
  ...
  return (
    <>
      <div className="mb-6 space-y-4">
        <div>
          <label className="eyebrow text-[var(--color-text-muted)] mb-2 block">Formato</label>
          <div className="flex gap-3">
            {(['png', 'jpg'] as const).map((f) => (
              <button
                ...
                className={`py-2 px-6 text-sm transition-all duration-200 ${
                  format === f
        ...
        <div>
          <label className="eyebrow text-[var(--color-text-muted)] mb-2 block">DPI: {dpi}</label>
          <input
            type="range"
            min={72}
            max={600}
