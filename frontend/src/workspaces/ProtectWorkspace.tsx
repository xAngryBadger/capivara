...
export function ProtectWorkspace() {
  ...
  const handleProtect = useCallback(async () => {
    ...
    if (restrictPrint) formData.append('restrict_print', 'true')
    ...
    try {
      ...
      const newName = files[0].name.replace(/\.pdf$/i, '_protegido.pdf')
      ...
      if (error instanceof Error && error.message === 'NO_API_URL') {
        ...
        setResults([{ original: files[0].name, status: 'error', error: 'Erro ao proteger. Verifique se o backend está online.' }])
      }
    } finally {
      setConverting(false)
    }
  }, [files, password, restrictPrint])

  return (
    <>
      <div className="mb-6 space-y-4">
        <div>
          <label className="eyebrow text-[var(--color-text-muted)] mb-2 block">Senha</label>
          <input
            type="password"
            value={password}
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          />
          <span className="text-sm text-[var(--color-text-muted)]">Restringir impressão</span>
        </label>
      </div>
