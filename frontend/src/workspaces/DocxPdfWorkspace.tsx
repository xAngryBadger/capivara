...
export function DocxPdfWorkspace() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/convert/docx-pdf'), {
        method: 'POST',
        body: formData,
        headers: apiHeaders(),
      })
      ...
      const newName = files[0].name.replace(/\.docx$/i, '.pdf')
  ...
  return (
    <>
      <FileUploader accept=".docx" files={files} onFilesChange={setFiles} label="Arraste um arquivo DOCX" />
